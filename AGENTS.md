...
- `./notes/wechat-session/`：课程群聊记录，按 `MM-DD/` 目录组织
- `./tasks/`：作业回答
- `./tasks-list.md`：待完成作业
...
3. 维护任务列表：
   ...
   2. 将任务解析到 `./tasks-list.md`。
      如果需要查看任务详情，先查找课程子模块对应 `session-*` 目录，再用 `./notes/wechat-session/` 辅助确认。
      **不需要提交到微信群的任务自动标记为已完成。**
   3. 任务描述必须以对应的 session 序号作为任务 ID；同一 session 下有多项任务时，在 session ID 后追加局部序号以保持唯一，例如 `session-1-1`、`session-1-2`。
      完成任务后只标 `[x]`，不删除。
      ```md
      ## 05-09 课程任务
      - [ ] session-1-1. 需要提交到微信群且未完成的任务
      - [x] session-1-2. 不需要提交到微信群的任务
      - [x] session-1-3. 需要提交到微信群且完成的任务
      ```
